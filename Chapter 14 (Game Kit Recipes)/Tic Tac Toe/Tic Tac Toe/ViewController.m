...
{
    [super viewDidLoad];
    UIBarButtonItem *playButton = [[UIBarButtonItem alloc] initWithTitle:@"Play" style:UIBarButtonItemStyleBordered target:self action:@selector(playGame:)];
...
- (void)playGame:(id)sender
{
    ...
    {
        UIAlertView *notLoggedInAlert = [[UIAlertView alloc] initWithTitle:@"Error" message:@"You must be logged into Game Center to play this game!" delegate:nil cancelButtonTitle:@"Dismiss" otherButtonTitles:nil];
    ...
    [_match loadMatchDataWithCompletionHandler:^(NSData *matchData, NSError *error)
     {
         dispatch_async(dispatch_get_main_queue(),^{
             ...
             self.statusLabel.text = [NSString stringWithFormat:@"%@'s turn", currentMark];
         });
     }];
}
...
{
    return [self.localPlayer.playerID isEqualToString:self.match.currentParticipant.playerID];
}
...
- (NSData *)encodeMatchData
{
    ...
    self.row1Col1Button.currentTitle, self.row1Col2Button.currentTitle, self.row1Col3Button.currentTitle,
    self.row2Col1Button.currentTitle, self.row2Col2Button.currentTitle, self.row2Col3Button.currentTitle,
    self.row3Col1Button.currentTitle, self.row3Col2Button.currentTitle, self.row3Col3Button.currentTitle
    ];
...
- (void)decodeMatchData:(NSData *)matchData
{
    ...
    [self.row1Col1Button setTitle:[stateArray objectAtIndex:1] forState:UIControlStateNormal];
    [self.row1Col2Button setTitle:[stateArray objectAtIndex:2] forState:UIControlStateNormal];
    [self.row1Col3Button setTitle:[stateArray objectAtIndex:3] forState:UIControlStateNormal];
    [self.row2Col1Button setTitle:[stateArray objectAtIndex:4] forState:UIControlStateNormal];
    [self.row2Col2Button setTitle:[stateArray objectAtIndex:5] forState:UIControlStateNormal];
    [self.row2Col3Button setTitle:[stateArray objectAtIndex:6] forState:UIControlStateNormal];
    [self.row3Col1Button setTitle:[stateArray objectAtIndex:7] forState:UIControlStateNormal];
    [self.row3Col2Button setTitle:[stateArray objectAtIndex:8] forState:UIControlStateNormal];
    [self.row3Col3Button setTitle:[stateArray objectAtIndex:9] forState:UIControlStateNormal];
}
...
- (IBAction)selectButton:(UIButton *)sender
{
    ...
    {
        UIAlertView *squareOccupiedAlert = [[UIAlertView alloc] initWithTitle:@"Invalid Move" message:@"You can only pick empty squares" delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil];
...
{
    if ([self equalMarksForButton1:self.row1Col1Button button2:self.row1Col2Button button3:self.row1Col3Button])
    {
        [self gameEndedWithWinner:self.row1Col1Button.currentTitle];
    }
    else if ([self equalMarksForButton1:self.row2Col1Button button2:self.row2Col2Button button3:self.row2Col3Button])
    {
        [self gameEndedWithWinner:self.row2Col1Button.currentTitle];
    }
    else if ([self equalMarksForButton1:self.row3Col1Button button2:self.row3Col2Button button3:self.row3Col3Button])
    {
        [self gameEndedWithWinner:self.row3Col1Button.currentTitle];
    }
    else if ([self equalMarksForButton1:self.row1Col1Button button2:self.row2Col1Button button3:self.row3Col1Button])
    {
        [self gameEndedWithWinner:self.row1Col1Button.currentTitle];
    }
    else if ([self equalMarksForButton1:self.row1Col2Button button2:self.row2Col2Button button3:self.row3Col2Button])
    {
        [self gameEndedWithWinner:self.row1Col2Button.currentTitle];
    }
    else if ([self equalMarksForButton1:self.row1Col3Button button2:self.row2Col3Button button3:self.row3Col3Button])
    {
        [self gameEndedWithWinner:self.row1Col3Button.currentTitle];
    }
    else if ([self equalMarksForButton1:self.row1Col1Button button2:self.row2Col2Button button3:self.row3Col3Button])
    {
        [self gameEndedWithWinner:self.row1Col1Button.currentTitle];
    }
    else if ([self equalMarksForButton1:self.row1Col3Button button2:self.row2Col2Button button3:self.row3Col1Button])
    {
        [self gameEndedWithWinner:self.row1Col3Button.currentTitle];
    }
...
{
    return button1.currentTitle.length > 0 && [button1.currentTitle isEqualToString:button2.currentTitle] && [button1.currentTitle isEqualToString:button3.currentTitle];
}

- (void)gameEndedWithWinner:(NSString *)mark
{
    NSString *message = [NSString stringWithFormat:@"%@ won!", mark];
    UIAlertView *gameOverAlert = [[UIAlertView alloc] initWithTitle:@"Game Over" message:message delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil];
    [gameOverAlert show];
    ...
    [self.match endMatchInTurnWithMatchData:matchData completionHandler:^(NSError *error) {
        if (error)
        {
...
{
    [self enableSquareButtons:NO];
    self.statusLabel.text = [NSString stringWithFormat:@"%@'s turn", [self opponentMark]];
    self.match.message = self.statusLabel.text;
    NSData *matchData = [self encodeMatchData];
    [self.match endTurnWithNextParticipants:@[[self opponentParticipant]] turnTimeout:GKTurnTimeoutDefault matchData:matchData completionHandler:^(NSError *error)
     {
         if (error)
         {
